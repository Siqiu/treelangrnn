...
threshold_func = 'soft1'	# threshold function, chose from [hard, soft1, soft2, dynamic]
threshold_mode = 'none'		# threshold mode, chose from [none, both, train, eval] (indicates when thresholding is applied)
...
threshold_max_radius = 0.5	# if threshold is decreased over time this is the starting value, otherwise this is the fixed radius
...
threshold_config = ThresholdConfig(threshold_func, threshold_mode, threshold_temp,
							threshold_min_radius, threshold_max_radius, threshold_decrease_radius,
							threshold_nlayers, threshold_nhid, threshold_lr) if not threshold_func == 'none' else None
